...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="GSFixer: Train & Fine-tune"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/finetune_gsfixer.yaml",
        help="Path to config file. Use 'config/train_gsfixer.yaml' for training from scratch.",
    )
    parser.add_argument(
        "--resume_run",
        action="store",
    ...
    model = MarigoldGSFixerPipeline.from_pretrained(
    ...
    trainer = trainer_cls(
        ...
        vis_dataloaders=[vis_loader],
        dual_input=cfg_data.train.get("dual_input", False),
    )
